...
// ===== EASTER EGG — type "daron" anywhere =====
let eggBuffer = '';
document.addEventListener('keydown', (e) => {
    eggBuffer = (eggBuffer + e.key.toLowerCase()).slice(-6);
    if (eggBuffer.includes('daron')) {
        eggBuffer = '';
        triggerMatrixRain();
    }
});
...
function triggerMatrixRain() {
    ...
    const chars = 'DARONSEROP₿⟠◈⬡01アイウエオカキクケコ'.split('');
    const fontSize = 15;
    ...
    setTimeout(() => {
        ...
        mctx.fillText('daron serop — class of 2030', mc.width / 2, mc.height / 2 + 18);
        mctx.textAlign = 'left';
    }, 1500);
...
// ===== CONSOLE EASTER EGG =====
console.log(
    '%c DS %c Daron Serop — armenghazaryan.am/daron-serop ',
